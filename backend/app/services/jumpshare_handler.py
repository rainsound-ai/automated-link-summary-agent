    ...
    for script in scripts:
        if script.string and "fileData" in script.string:
            try:
                ...
                match = re.search(
                    r"fileData\s*=\s*(\{.*?\});", script.string, re.DOTALL
                )
                if match:
                    ...
                    if isinstance(data, dict):
                        if "download_url" in data:
                            ...
                            for item in data["items"]:
                                if "download_url" in item and item.get(
                                    "type", ""
                                ).startswith("video"):
...
async def get_videos_from_jumpshare_links(
    jumpshare_links: List[str],
) -> List[UploadFile]:
    videos = []
    headers = {
        ...
        for link in jumpshare_links:
            try:
                ...
                if response.status_code != 200:
                    logger.error(
                        f"Failed to download video from {link}: {response.status_code}"
                    )
                    continue
    ...
    if not videos:
        raise HTTPException(
            status_code=500, detail="Failed to download any videos from Jumpshare links"
        )
