...
async def summarize_transcription(transcription: str, prompt: str) -> str:
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Unexpected error during summarization"
        )
...
async def decomposed_summarize_transcription_and_upload_to_notion(
    ...
    for user in users:
        ...
        user_toggle_id = await create_toggle_block(
            page_id, f"{user.title()}'s Summary", "green"
        )
        ...
        for attempt in range(max_attempts):
            try:
                ...
                full_prompt = prompt_manager.get_prompt_for_user(
                    user, is_llm_conversation, is_jumpshare_link
                )
                if feedback:
                    full_prompt += f"\n\nPrevious feedback:\n{feedback}"

                # Generate summary
                current_summary = await summarize_transcription(
                    transcription, full_prompt
                )
                ...
                logger.error(
                    f"🚨 Attempt {attempt + 1} for user {user} failed: {str(e)}"
                )
                if attempt == max_attempts - 1:
                    logger.error(
                        f"Failed after {max_attempts} attempts for user {user}"
                    )
                continue
    ...
    if llm_conversation_file_name:
        ...
        await update_notion_title_for_summarized_item(
            page_id, f"LLM Conversation: {formatted_name}"
        )
    else:
